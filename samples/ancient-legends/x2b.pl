...
open( A, "$readFile" )
  || die(
"Can't open read-file $readFile. If you want options, or how to write an extractor file, push -?"
  );
while ( $line = <A> ) {
  if ( $line =~ /^;/ ) { last; }
  if ( $line =~ /^#/ ) { next; }
  ...
  if ( $line eq "ir" or $line eq "reset" ) {
    $iconIdx  = 0;
    %iconHash = ();
    next;
  }
    ...
    if ( ( $mapRead == 1 ) && ( $l =~ /<row>/ ) ) {
      ...
      for (@tiles) {
        if ( !defined( $iconHash{$_} ) ) {
          ...
          if ( $iconIdx == 257 ) {
            die(
"Too many icons read in at line $.. Use ir in a line somewhere, or open another file."
            );
          }
        }
        if ($debug) {
